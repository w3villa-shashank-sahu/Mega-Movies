import { Search, HeartIcon, EllipsisVertical } from 'lucide-react';
...
const AppBar = () => {
  ...
  return (
    <nav className="bg-gray-900 px-4 pb-0 sm:pb-4 w-full sticky top-0 z-1 h-fit justify-center flex flex-col sm:flex-row">
      <div className="max-w-7xl w-full flex justify-between items-center h-full pt-3">
        <h1 className="text-white text-[clamp(23px,3.5vw,30px)] md:text-3xl font-bold cursor-default"
        onClick={()=>{navigate(MyRoutes.home)}}>
          Mega Movies
        </h1>
        <div className="flex items-center text-sm md:text-base gap-1 sm:gap-3">
          {/* wishlist button */}
          ...
          onClick={()=>{navigate(MyRoutes.wishlist)}}>
            ...
            <HeartIcon size={'clamp(16px,5vw,20px)'} />
          </button>
          {/* search bar */}
          <div className="rounded-lg bg-gray-800 text-white h-8 items-center flex">
              ...
              <button onClick={handleSearch} className='h-full px-2 bg-gray-700 rounded-xl sm:rounded-r-xl cursor-pointer hover:bg-gray-600'>
                <Search size={'clamp(16px,5vw,20px)'} />
              </button>
          </div>
          {/* options */}
          <EllipsisVertical className='text-white cursor-pointer' size={'clamp(20px,5vw,20px)'} onClick={()=>{
            console.log('hello');
      ...
      {/* second search bar */}
      <div className={`w-full flex sm:hidden my-2  rounded-md text-sm overflow-hidden outline-[#fff7] outline-1 text-white duration-300 ease-in-out transition-all ${showInput ? 'h-auto opacity-100 mb-4' : 'h-0 opacity-0'}`}>
        <input
          type="text"
          id="searchInput2"
          placeholder="Search movies"
